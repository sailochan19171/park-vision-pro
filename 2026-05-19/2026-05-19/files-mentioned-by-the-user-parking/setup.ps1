...
$keyOrder = @(
    ...
    'GATE_LOCATION_NAME', 'GATE_ADDRESS_LINE', 'GATE_LATITUDE', 'GATE_LONGITUDE', 'WATERMARK',
...
# ----- Step 3b. Write the correct run.bat -----
# We embed the known-good run.bat content directly here so the scheduled
# task always launches the current version. Skips any dependency on
# git pull / manual file copy / GitHub reachability. Every SETUP.bat
# invocation re-writes this file, so it's self-healing.
Write-Banner "Step 3b of 5 -- Regenerating run.bat (self-healing)" 'Cyan'
$runBatPath = Join-Path $PSScriptRoot 'run.bat'
$runBatContent = @'
@echo off
REM =============================================================
...
REM
REM Regenerated by SETUP.bat on every run so it stays fresh.
REM =============================================================
...
# Try to overwrite. If the file is locked (e.g. task is running),
# stop the task, kill python.exe, and retry.
try {
    Set-Content -Path $runBatPath -Value $runBatContent -Encoding ascii -Force
    Write-Host "[OK] run.bat regenerated at $runBatPath" -ForegroundColor Green
} catch {
    Write-Host "[..] run.bat is locked -- stopping task + killing python.exe first..." -ForegroundColor Yellow
    Stop-ScheduledTask -TaskName VayAccessOnSiteAgent -ErrorAction SilentlyContinue
    Get-Process python -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
    Start-Sleep 3
    Set-Content -Path $runBatPath -Value $runBatContent -Encoding ascii -Force
    Write-Host "[OK] run.bat regenerated (after unlock)" -ForegroundColor Green
}
